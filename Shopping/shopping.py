...
def load_data(filename):
    ...
    for index, row in df.iterrows():
        record = [row['Administrative'], row['Administrative_Duration'], row['Informational'], row['Informational_Duration'], row['ProductRelated'],
                ...
                row['OperatingSystems'], row['Browser'], row['Region'], row['TrafficType'], int(row['VisitorType'] == 'Returning_Visitor'), int(row['Weekend'])]
...
def evaluate(labels, predictions):
    ...
    tp = sum(np.logical_and(labels==1,predictions==1))
    fp = sum(np.logical_and(labels==0,predictions==1))
    
    tn = sum(np.logical_and(labels==0,predictions==0))
    fn = sum(np.logical_and(labels==1,predictions==0))
